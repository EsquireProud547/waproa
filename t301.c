/*
 * TCP WAPROA v3.0.1 (Production Release)
 * ============================================================================
 * 版本: v3.0.1
 ...
 * - [修复] RTT=0安全处理(防止高速网络rtt_min初始化停滞)
 * - 全内核版本兼容(2.6.13 - 6.x)
 * - BDP自适应带宽估计与空闲指数衰减
 * ============================================================================
 */
...
/* tcp_is_cwnd_limited() 2.6.13-2.6.25兼容 */
...
/* 拥塞响应参数 */
#define WA_LOSS_RETENTION_NUM		7		/* 70% */
#define WA_LOSS_RETENTION_DEN		10
#define WA_ECN_REDUCTION_NUM		8		/* 80% */
...
struct waproa {
	...
	u32	last_safe_cwnd;		/* BDP安全窗口参考 */
	u32	rtt_reset_time;		/* RTT老化计时器 */
	u32	cnt_and_flags;		/* 位域: 计数器+标志+日志+自定义 */
};
...
/* [修复] RTT=0安全处理: 高速网络下rtt_us可能为0(亚微秒ACK)
 * 确保rtt_min能正确初始化，但避免已有值时被0污染
 */
static void waproa_update_rtt(struct waproa *ca, u32 rtt_us, u32 now)
{
	/* 条件: 正常RTT>0，或rtt_min尚未初始化(0)时强制设置最小值 */
...
{
	pr_info("TCP WAPROA v3.0.1: RFC-compliant BDP congestion control
");
	pr_info("  Memory: %zu bytes, HZ-independent, RTT=0-safe
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("TCP WAPROA v3.0.1 - Production BDP Congestion Control");
